...
namespace ImageRenamer
{
    ...
    public class SemanticVersion : IComparable<SemanticVersion>
    {
        ...
        public SemanticVersion(string version)
        {
            ...
            Major = parts.Length > 0 && int.TryParse(parts[0], out int major) ? major : 0;
            Minor = parts.Length > 1 && int.TryParse(parts[1], out int minor) ? minor : 0;
            ...
            while (retryCount <= MaxRetryCount)
            {
                try
                {
                    using (HttpClient httpClient = CreateHttpClient())
                    {
                        ...
                        return JsonSerializer.Deserialize<VersionConfig>(json);
                    }
                }
                catch (System.Net.Http.HttpRequestException ex)
                {
                    retryCount++;
                    string errorMsg = $"[重试 {retryCount}/{MaxRetryCount}] HTTP请求失败：\n" +
                    ...
                    {
                        ShowErrorMessage($"HTTP请求失败：状态码 {ex.StatusCode}\n错误信息：{ex.Message}");
                        return null;
                    }
                ...
                {
                    retryCount++;
                    string errorMsg = $"[重试 {retryCount}/{MaxRetryCount}] 请求超时：\n" +
                                     $"异常类型：{ex.GetType().Name}\n" +
                                     $"错误信息：{ex.Message}\n" +
                                     $"堆栈跟踪：{ex.StackTrace}";
                ...
                {
                    retryCount++;
                    string errorMsg = $"[重试 {retryCount}/{MaxRetryCount}] 未知错误：\n" +
                                     $"异常类型：{ex.GetType().Name}\n" +
                                     $"错误信息：{ex.Message}\n" +
                                     $"堆栈跟踪：{ex.StackTrace}";
                    ...
                    if (retryCount > MaxRetryCount)
                    {
                        ShowErrorMessage($"未知错误：{ex.GetType().Name}\n{ex.Message}");
                        return null;
                    }
                    await Task.Delay(1000 * retryCount);
                }
    ...
    public class UpdateForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            lblVersion.Text = $"最新版本: v{config.latest_version}";
            ...
            lblDate.Text = $"发布日期: {config.release_date}";
    ...
    public partial class MainForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            lblVersion.Text = $"当前版本: v{UpdateService.CurrentVersion}";
